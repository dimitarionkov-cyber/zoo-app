...
- [x] Home's active-visit banner shows a pulsating green dot (fixed contrast — dot now sits on a light card circle instead of blending into a solid-green one) plus a live "N видяно/видени" count
- [x] `/visited` simplified — dropped the overview map and the full lifetime animal list; shows the last completed visit's own animal list (with per-visit times) instead, alongside the lifetime count/progress bar and visit history
