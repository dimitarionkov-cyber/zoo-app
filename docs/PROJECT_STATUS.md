...
- **Visits are a list, not a single active/last slot**: `visits: [{ id, startedAt, endedAt, seen: { [animalId]: { at, firstTime } } }]` replaced the earlier `activeVisit`/`lastVisit` single-slot fields (still exposed from `DataContext`, but derived via `useMemo` from `visits`). Lifetime "ever seen" (`visited`) stays a separate, unchanged map — checkmarks across the app intentionally keep reading from it rather than from any one visit, so they never appear to "reset."
- **A page reading the active visit must look it up by id, not hold the object**: `VisitPage` captures which visit it's for once at mount (`visits.find(v => !v.endedAt)?.id`), then looks that record up fresh from the live `visits` array every render. Ending the visit only sets its `endedAt` — the record is never removed from the array — so the lookup never goes null out from under the page mid-navigation (the same class of race hit earlier when `activeVisit` was a single field that got set to `null` on end).
